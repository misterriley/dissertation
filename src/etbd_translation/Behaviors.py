...
from etbd_translation.Mutator import Mutator
from etbd_translation.PopulationSaver import PopulationSaver
from etbd_translation.Recombinator import Recombinator
from etbd_translation.SampleWoutReplace import SampleWoutReplace
from etbd_translation.Selector import Selector
...
class Behaviors(object):
    ...
    def __init__(self, stuBehaviorsInfo):
        ...
        self.m_blnForceClone = None  # Flag to force cloning.  Set to true when the BehaviorToClone property is set.  Must be reset after the
    ...
    def set_high_phenotype(self, value):
        ...
        self.m_bitsInHighPhenotype = len(BinaryConvertBoolean.convert_from_base_10(self.m_highPhenotype)) - 1
...
# Region " Procedures for creating new generations."
    ...
    def create_new_generation(self, blnSelection):
        ...
        if blnSelection and self.m_blnForceClone:
            ...
            for _ in range(1, intNumToReplace + 1):  # intNumToReplace was assigned the correct value
            ...
            for _ in range(1, intNumToReplace2 + 1):  # intNumToReplace2 was assigned the correct value
                #                          when it was dimensioned in this procedure.
    ...
    def add_mutation(self):
        ...
        for index in range(len(self.m_behavior_list)):
            ...
            if integerValueReturned != integerValuePassed:
                ...
                self.set_behavior(index, Behavior(integerValueReturned))
                self.get_behavior(index).pad_to(self.m_bitsInHighPhenotype)
...
# Region " Collection utilities"
    ...
    def population_with_random_behaviors(self):
        ...
        for _ in range(1, self.get_num_behaviors() + 1):
            objBehavior = Behavior(self.m_objRandom.get_rectangular_integer())
            # Pad with leading zeros if necessary
            objBehavior.pad_to(self.m_bitsInHighPhenotype)
            self.add_behavior(objBehavior)
